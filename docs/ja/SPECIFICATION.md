...
| **ファイル I/O**         | 実PCファイル保存ボタン                          | LocalStorage 作成中ノートからワンクリックで実ファイル保存ダイアログを起動                               |
| **ファイル I/O**         | 保存ダイアログキャンセル時ステータス復元        | キャンセル時に保存ステータスを正常状態 (`saved_local` / `saved_file` / `unsaved`) へ完全復元            |
